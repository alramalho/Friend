...
void callApiOnKeywords(Map<int, Map<String, dynamic>> current) {
  const List<String> demoKeywords = [
    "Jarvis",
    "Jardis",
    "Jurgies",
    "Jurgys",
  ...
  const demoApiUrl = "https://k33oesgoomcnyqsr4ry7nvb6em0qzzdq.lambda-url.eu-central-1.on.aws/friend";

  current.forEach((speaker, data) {
    String transcript = data['transcript'] ?? '';
    for (String keyword in demoKeywords) {
      if (transcript.contains(keyword)) {
        
        // Call the demo API
        http.post(
          Uri.parse(demoApiUrl),
          body: jsonEncode({'keyword': keyword, 'transcript': transcript}),
    ...
    Tuple4<IOWebSocketChannel?,
        ...
        await bleReceiveWAV(
            ...
            onWebsocketConnectionFailed: (err) {
              ...
              debugPrint('Websocket Unable To Connect: $err');
